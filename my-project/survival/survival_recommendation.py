...
from survival_LCCS_2 import *
from survival_LCCS_3 import *
    ...
    def train(self, X_test, y_test, num_epoch = 5000, iteration=-1):
        ...
        for i in range(num_epoch):
            ...
            if (iteration != -1) and (i % iteration == 0):
                ...
                if valid_CI > best_CI:
                    best_CI = valid_CI
                self.saver.save(self.sess, "model_recommendation_2/model.ckpt")
...
if __name__ == '__main__':
    ...
    with open('survival/data/train_LCCS_2_onehot.csv') as f:
        lines = csv.reader(f)
        head = next(lines)
        for line in lines:
            X_train.append(line)

    with open('survival/data/train_LCCS_2_label.csv') as f:
        lines = csv.reader(f)
        head = next(lines)
        for line in lines:
    ...
    model = snn(123,[64,32,16],1, optimizer='adam')
    model.load_model('survival/model_recommendation_2/model.ckpt')
    T0_1, ST_1 = model.survival_function(np.array([patient]),algo="wwe",base_X=np.array(x_train), base_label={'e':np.array(y_train['e']), 't':np.array(y_train['t'])})
    model.close()
    ...
    with open('survival/data/train_LCCS_3_onehot.csv') as f:
        lines = csv.reader(f)
        head = next(lines)
        for line in lines:
            X_train.append(line)

    with open('survival/data/train_LCCS_3_label.csv') as f:
        lines = csv.reader(f)
        head = next(lines)
        for line in lines:
            train_e.append(int(line[0]))
    ...
    model = snn(123,[64,32,16],1, optimizer='adam')
    model.load_model('survival/model_recommendation_3/model.ckpt')
    T0_2, ST_2 = model.survival_function(np.array([patient]),algo="wwe",base_X=np.array(x_train), base_label={'e':np.array(y_train['e']), 't':np.array(y_train['t'])})
    model.close()

    #   print([T0.tolist(),ST[0].tolist()])
    print([[T0_1.tolist(),ST_1[0].tolist()],[T0_2.tolist(),ST_2[0].tolist()]])
