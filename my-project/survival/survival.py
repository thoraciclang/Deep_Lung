...
class snn(object):
    ...
    def load_model(self, model_file):
        self.saver = tf.train.import_meta_graph('model/my-model-290.meta')
    ...
    def train(self, X_test, y_test, num_epoch = 5000, iteration=-1):
        ...
        for i in range(num_epoch):
            ...
            if (iteration != -1) and (i % iteration == 0):
                ...
                if valid_CI > best_CI:
                    best_CI = valid_CI
                    self.saver.save(self.sess, "model/my-model", global_step=i)
...
data[:,0] = np.array(range(0,data.shape[0]))
...
# X_train, X_test, y_train_index, y_test_index = train_test_split(new_data,index,test_size=0.2,random_state=1)
y_train_index = []
y_test_index = []
fp1 = open('train_index.txt','r')
contents1 = fp1.readlines()
for n in contents1:
    n = n.strip('\n')
    y_train_index.append(int(n))
fp1.close()

fp2 = open('test_index.txt','r')
contents2 = fp2.readlines()
for n in contents2:
    n = n.strip('\n')
    y_test_index.append(int(n))
fp2.close()

X_train = [new_data[i] for i in y_train_index]
X_test = [new_data[i] for i in y_test_index]
y_train = {'e':[new_e[i] for i in y_train_index], 't':[new_t[i] for i in y_train_index]}
y_test = {'e': [new_e[i] for i in y_test_index], 't': [new_t[i] for i in y_test_index]}

# fp1 = open('train_index.txt','w+')
# for i in y_train_index:
#     fp1.write(str(i))
#     fp1.write('\n')
#
# fp1.close()
# fp2 = open('test_index.txt','w+')
...
model = snn(len(x_train[0]),[64,32,16],1, optimizer='adam')
model.load_data(np.array(x_train),{'e':np.array(y_train['e']), 't':np.array(y_train['t'])})
model.generate_graph()
model.train(np.array(x_test),{'e':np.array(y_test['e']), 't':np.array(y_test['t'])}, num_epoch=300, iteration=1)

fp3 = open('results.txt','w+')
fp3.write('Lobectomy, None, Sublobar, Peumonectomy')
fp3.write('\n')
