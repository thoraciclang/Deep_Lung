...
df_ = df = pd.read_csv('./data/train_LCCS_3.csv', encoding='utf-8')
...
df[['Sex','Histologic Type ICD-O-3', 'Histologic Type 2', 'Grade', 'RX Summ--Scope Reg LN Sur (2003+)', 'CS extension (2004+)', 'CS mets at dx (2004+)', 'Stage', 'T8', 'N8', 'M8', 'Marital status at diagnosis', 'Lung - Pleural/Elastic Layer Invasion (PL) by H and E or Elastic Stain', 'Lung - Separate Tumor Nodules - Ipsilateral Lung', 'Lung - Surgery to Other Regional/Distant Sites (1998+)']] = df[['Sex','Histologic Type ICD-O-3', 'Histologic Type 2', 'Grade', 'RX Summ--Scope Reg LN Sur (2003+)', 'CS extension (2004+)', 'CS mets at dx (2004+)', 'Stage', 'T8', 'N8', 'M8', 'Marital status at diagnosis', 'Lung - Pleural/Elastic Layer Invasion (PL) by H and E or Elastic Stain', 'Lung - Separate Tumor Nodules - Ipsilateral Lung', 'Lung - Surgery to Other Regional/Distant Sites (1998+)']].astype(object)
print(df.info())
print(df.head())
label = df[['LCCS', 'Survival months']]
df = df.drop(['LCCS', 'Survival months'], axis=1)
label.to_csv('./data/train_LCCS_3_label.csv', encoding='utf-8', index=False)
...
df.to_csv('./data/train_LCCS_3_onehot.csv', encoding='utf-8', index=False)
